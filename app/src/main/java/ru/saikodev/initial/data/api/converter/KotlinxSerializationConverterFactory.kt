 ...
 *
 * Uses pure Java reflection to resolve serializers from @Serializable companion objects,
 * which works across all Kotlin versions (1.x and 2.0+) without deprecated KClass APIs.
 */
class KotlinxSerializationConverterFactory(
     ...
     * Uses pure Java reflection to find the companion class and its serializer() method,
    ...
    private fun resolveSerializer(type: Type): KSerializer<Any> {
        ...
        try {
            // Find the Companion class via Java reflection (works on all Kotlin versions)
